...
#elif defined(MVSX)
#define SDL_RS90_WIDTH  1280
#define SDL_RS90_HEIGHT 1024
#define SDL_DINGUX_MENU_WIDTH 640
#define SDL_DINGUX_MENU_HEIGHT 512
#elif defined(ASP)
#define SDL_RS90_WIDTH  1280
#define SDL_RS90_HEIGHT 720
#define SDL_DINGUX_MENU_WIDTH 640
#define SDL_DINGUX_MENU_HEIGHT 360
#elif defined(CRS)
#define SDL_RS90_WIDTH  1024
#define SDL_RS90_HEIGHT 768
#define SDL_DINGUX_MENU_WIDTH 512
#define SDL_DINGUX_MENU_HEIGHT 384
#else
      ...
      unsigned src_pitch)
{
   ...
   int max_scale_factor = SDL_RS90_HEIGHT / height;  // Calculate the maximum scaling factor based on height
   unsigned padded_width = 0;
   int scale_factor = 0;
   bool was_padded = false;

   for (int factor = max_scale_factor; factor >= 1; factor--) {
      // Calculate the next multiple of the scale factor larger than or equal to width
      unsigned tmp_padded_width = ((SDL_RS90_WIDTH + factor - 1) / factor);
      if (tmp_padded_width >= width) {
         scale_factor = factor;
         padded_width = (factor == 3? tmp_padded_width - 32: tmp_padded_width); // for 3x scale we substract 32 px for margin
         break;
      }
   }

    uint16_t* padded_src = (uint16_t*)malloc(padded_width * height * sizeof(uint16_t));
    if (!padded_src) {
        return;  // Consider adding error handling or logging here
    }
    int pre_scale_padding = (padded_width - width) / 2;
    for (unsigned y = 0; y < height; y++) {
        memset(padded_src + y * padded_width, 0, pre_scale_padding * sizeof(uint16_t));
        memcpy(padded_src + y * padded_width + pre_scale_padding, src + y * in_stride, width * sizeof(uint16_t));
        memset(padded_src + y * padded_width + pre_scale_padding + width, 0, pre_scale_padding * sizeof(uint16_t));
    }
    
    src = padded_src;
    src_pitch = padded_width * 2;
    was_padded = true;

    unsigned scaled_width = padded_width * scale_factor;
    unsigned scaled_height = height * scale_factor;
    int pad_x = (SDL_RS90_WIDTH - scaled_width) / 2;
    int pad_y = (SDL_RS90_HEIGHT - scaled_height) / 2;
    uint16_t *in_ptr = src;
    size_t out_stride = SDL_RS90_WIDTH;
    uint16_t *out_ptr = (uint16_t*)(vid->screen->pixels) + pad_x + pad_y * out_stride;

    switch (scale_factor) {
        case 1: scale1x_n16(in_ptr, out_ptr, padded_width * 2, height, src_pitch, out_stride * 2); break;
        case 2: scale2x_n16(in_ptr, out_ptr, padded_width * 2, height, src_pitch, out_stride * 2); break;
        case 3: scale3x_n16(in_ptr, out_ptr, padded_width * 2, height, src_pitch, out_stride * 2); break;
        case 4: scale4x_n16(in_ptr, out_ptr, padded_width * 2, height, src_pitch, out_stride * 2); break;
        case 5: scale5x_n16(in_ptr, out_ptr, padded_width * 2, height, src_pitch, out_stride * 2); break;
    }
      ...
      unsigned src_pitch)
{
    ...
    // Check and pad if resolution is 304x224
    if (width == 304 && height == 224) {
        uint16_t* padded_src = (uint16_t*)malloc(320 * 224 * sizeof(uint16_t));
        if (!padded_src) {
            return;
        }

        for (unsigned y = 0; y < 224; y++) {
            memcpy(padded_src + y * 320, src + y * in_stride, 304 * sizeof(uint16_t));
            memset(padded_src + y * 320 + 304, 0, 16 * sizeof(uint16_t));
        }
    ...
    unsigned scaled_width = width * 4;
    unsigned scaled_height = height * 4;

    int pad_x = (1280 - scaled_width) / 2;
    int pad_y = (1024 - scaled_height) / 2;

    uint16_t *in_ptr = src;
    size_t out_stride = 1280;
    uint16_t *out_ptr = (uint16_t*)(vid->screen->pixels) + pad_x + pad_y * out_stride;

    scale4x_n32(in_ptr, out_ptr, width * 2, height, src_pitch, out_stride * 2);
